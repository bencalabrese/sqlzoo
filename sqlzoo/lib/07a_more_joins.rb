...
def title_tracks
  ...
  FROM
    tracks
    JOIN albums
      ON albums.asin = tracks.album AND tracks.song = albums.title;
  SQL
end
...
def rock_superstars
  ...
  execute(<<-SQL)
  SELECT
    uniq_rock_albums.artist, COUNT(*)
  FROM
    (
      SELECT
        DISTINCT albums.asin, albums.artist
      FROM
        styles
        JOIN albums
...
def expensive_tastes
  ...
  execute(<<-SQL)
  SELECT
    styles.style, (SUM(album_track_count.price)/SUM(album_track_count.num_tracks)) AS price_per_track
  FROM
    (
      SELECT
    ...
    JOIN styles
      ON styles.album = album_track_count.asin
  GROUP BY
    styles.style
  ORDER BY
